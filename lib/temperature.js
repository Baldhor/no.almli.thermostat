...
let resolveOnoff = function (aDevice, temperature, targetTemp, settings, opts, contactAlarm) {
    ...
    let onoff = undefined;
    if (contactAlarm) {
        aDevice.log(`has contactAlarm`);
        onoff = aDevice.getCapabilityValue('vt_onoff') === (invert !== true) ? (invert === true) : undefined;
    } else {
        if (!mainOnoff) {
	    // Using default target temperature
            targetTemp= settings.default_target_temperature
            aDevice.log(`using default target temperature of ${targetTemp}`);
        }
